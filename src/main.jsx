...
function Discover({ catalog, items, favorites, toggleFavorite, category, setCategory, query, setQuery, openPack, loading, reload, showToast, activeSetup, restoreVanilla }) {
  ...
  return <div className="library-shell"
    ...
    <PageHead title="Library" description="Your local FiveM packs." action={<div className="head-actions"><button className="text-btn library-add" onClick={choose} disabled={adding}>{adding ? <LoaderCircle size={14} className="spin"/> : <Plus size={14}/>}Add pack</button><button className="icon-btn ghost" title="Scan again" aria-label="Scan packs again" onClick={reload}><RotateCw size={15}/></button></div>} />
    ...
    <div className="toolbar"><label className="search"><Search size={16}/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search packs"/></label><span className="count">{catalog.items.length} packs</span></div>
...
function SettingsView({ settings, packsDir, reload, showToast, restoreVanilla, verifyPaths, pathCheck, appInfo, security, securityLoading, verifySecurity }) {
  ...
  return <>
    ...
    <section className="plain-section">
      <div className="section-heading"><h2>ReShade</h2><p>Install and enable ReShade for FiveM.</p></div>
      <div className="restore-row">
        <div><strong>Install ReShade</strong><span>Choose the official setup. Soren handles GTA V, shaders, and FiveM.</span></div>
        <button className="text-btn" onClick={() => window.appAPI.downloadReshade().catch((error) => showToast(friendlyError(error), 'error'))}><ExternalLink size={14}/>Get setup</button>
        <button className="primary-btn" disabled={reshadeBusy} onClick={launchReshade}>{reshadeBusy ? <LoaderCircle size={14} className="spin"/> : <Download size={14}/>}Install</button>
      </div>
      <div className="restore-row">
        <div><strong>FiveM status</strong><span>{reshadeStatus?.state === 'blocked' ? 'FiveM blocked ReShade. Read and acknowledge its crash warning to enable it.' : reshadeStatus?.state === 'acknowledged' ? 'Enabled. Restart FiveM, then press Home.' : reshadeStatus?.message || 'Launch FiveM once, then check its log.'}</span></div>
        <button className="secondary-btn" disabled={reshadeBusy} onClick={checkReshadeStatus}><ScanSearch size={14}/>Check</button>
        {reshadeStatus?.state === 'blocked' && <button className="primary-btn" disabled={reshadeBusy} onClick={acknowledgeReshade}>Acknowledge warning</button>}
      </div>
      {reshadeStatus?.state === 'blocked' && <p className="logic-note">FiveM warns that ReShade 5 or higher may cause crashes. Soren uses the acknowledgement line from your own FiveM log and backs up CitizenFX.ini.</p>}
      <details className="settings-details">
        <summary>Paths and repair</summary>
        <div className="restore-row"><div><strong>Game folders</strong><span>Find GTA V and FiveM plugins.</span></div><button className="secondary-btn" disabled={reshadeBusy} onClick={prepareReshade}><FolderOpen size={14}/>Check folders</button></div>
        {reshadePaths && <div className="setting-list">{[['FiveM plugins', reshadePaths.plugins], ['GTA V', reshadePaths.gta]].map(([label, result]) => <div className="setting-row" key={label}><FolderOpen size={18}/><div className="setting-copy"><strong>{label}</strong><span>{result.path || 'Not detected'}</span></div><span className={`path-state ${result.status === 'missing' ? 'missing' : 'ok'}`}>{result.status === 'created' ? 'Created' : result.status === 'found' ? 'Found' : 'Not found'}</span></div>)}</div>}
        {reshadeInstalled && <div className="setting-list"><div className="setting-row"><CircleCheck size={18}/><div className="setting-copy"><strong>GTA V DLL verified</strong><span>{reshadeInstalled.gtaDll}</span></div></div><div className="setting-row"><CircleCheck size={18}/><div className="setting-copy"><strong>FiveM plugin DLL verified</strong><span>{reshadeInstalled.pluginDll}</span></div></div></div>}
        <div className="restore-row"><div><strong>Repair FiveM files</strong><span>Copy an existing GTA V ReShade install into FiveM.</span></div><button className="secondary-btn" disabled={reshadeBusy} onClick={finishReshade}><RotateCw size={14}/>Repair</button></div>
      </details>
    </section>
